...
async def _send_linkedin_message(state, context, results, step):
    target_id = context.get("linkedin_id", "123")
    content = context.get("message", "Hello from Ovanthra!")
    results[step] = await send_social_message(state, "linkedin", target_id, content)

async def _send_slack_message(state, context, results, step):
    target_id = context.get("slack_id", "U123")
    content = context.get("message", "Hello from Ovanthra!")
    results[step] = await send_social_message(state, "slack", target_id, content)

async def _linkedin_connect(state, context, results, step):
    target_id = context.get("linkedin_id", "123")
...
class ExecutorNode:
    async def __call__(self, state: AgentState) -> Dict[str, Any]:
        ...
        # [NEW] Notify user via WebSocket about current step
        user_id = state.get("user_id")
        if user_id:
            import httpx
            try:
                # We use a background task or just fire and forget
                # For this implementation, we'll hit the gateway internal notify
                payload = {
                    "type": "agent_monologue",
                    "step": step,
                    "message": f"🤖 Agent is now executing: {step.replace('_', ' ')}...",
                    "timestamp": __import__("datetime").datetime.utcnow().isoformat()
                }
                # Using httpx.post with a short timeout to avoid blocking execution
                headers = {"Authorization": f"Bearer {state.get('internal_token')}"}
                httpx.post(f"http://api-gateway:8000/api/v1/internal/notify/{user_id}", json=payload, headers=headers, timeout=0.1)
            except:
