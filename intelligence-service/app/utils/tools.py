...
async def send_social_message(state: dict, platform: str, target_id: str, content: str):
    print(f"[Tool] Sending {platform} message to {target_id}")
    async with get_agent_client(state) as client:
        res = await client.post(
            f"{settings.EMAIL_SERVICE_URL}/api/v1/social/send",
            json={"platform": platform, "target_id": target_id, "content": content}
        )
        res.raise_for_status()
        return res.json()

async def linkedin_connect(state: dict, target_id: str, note: str = None):
    print(f"[Tool] Sending LinkedIn connect to {target_id}")
    async with get_agent_client(state) as client:
        res = await client.post(
            f"{settings.EMAIL_SERVICE_URL}/api/v1/social/linkedin/connect",
            params={"target_id": target_id, "note": note}
        )
        res.raise_for_status()
        return res.json()
